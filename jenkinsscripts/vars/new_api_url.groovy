...
def call (config = [:]) {
   ...
   config = config as NewApiUrlConfig
   ...
   azureRegistryUtils.withAzureRegistry {
      docker.image('topaasAPIM.azurecr.io/apimanagementimage').inside('--entrypoint ""') {
         ...
         withCredentials([usernamePassword(credentialsId: config.servicePrincipal, usernameVariable: 'servicePrincipalId', passwordVariable: 'servicePrincipalPassword'),
            ...
            body = createBody(apiDefinitionId, config.apiId, config.productOwner, servicePrincipalId, azureDevOpsUser, azureDevOpsToken, config.swaggerUrl, config.urlSuffix, config.apiFormat, config.slackResponseUri)
...
private static def createBody(apiDefinitionId, apiId, productOwner, servicePrincipalApplicationID, username, accessToken, swaggerUrl, urlSuffix, apiFormat, uri) {
   return """@"
        {
