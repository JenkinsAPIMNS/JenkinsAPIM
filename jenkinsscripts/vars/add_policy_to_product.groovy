...
def call (Map config){
   config = config as AddPolicyToProductConfig
   ...
   azureRegistryUtils.withAzureRegistry{
      docker.image('topaasAPIM.azurecr.io/apimanagementimage').inside('--entrypoint ""') {
         ...
         powershellUtils.runPowershellCommand """
            ...
            Set-AzApiManagementPolicy -Context \$ApiMgmtContext -ProductId "${config.productid}" -PolicyFilePath "${config.policyFilePathProd}"
         """
      }
   }
}        
